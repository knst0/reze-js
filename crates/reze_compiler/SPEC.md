...
### O7. `<Show>` как условие (`optimize`)

`<Show>` рантайма (по символу, §8.0; `Show` из `reze-js` или `@rezejs/dom/flow`) — ребёнок нативного
элемента с атрибутом `when`, необязательным `fallback` и без других атрибутов и spread, ровно одним
ребёнком (не считая пустого текста) — компилируется как условие §7.5 `when ? child : fallback`
(`Op::Memo` от `!!(when)` и вставка), если ребёнок и `fallback` — JSX (возможно в `{}`) или выражения без
реактивных чтений: `<Show>` строит ветки без отслеживания, и условие тогда не отслеживает больше.
Функция-ребёнок, строковый `fallback`, несколько детей или другие атрибуты оставляют компонент. Каждое
применение даёт `info` `SHOW_INLINED`.

Рантайм: `Show` держит один `computed` истинности и один для ветки; memo значения для функции-ребёнка
создаётся при первом чтении геттера.
