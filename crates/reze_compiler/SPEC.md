...
- Переключатели: если свернуть статически нельзя, но каждый источник — строка, объект со статическими
  ...
  `toggleClass(el, "key", !!(value), prev)` (привязка, если значение реактивно, иначе один раз).
  `toggleClass` трогает DOM, только если состояние сменилось; отсутствие `prev` считается «выключен». Ключ
  переключателя — один токен без пробелов. Если токен одновременно статический и переключаемый,
  переключается дважды или выключен литералом `false`, компилируется общий путь `className`. Server
  рендерит переключатели через `Op::ServerClass`: `ssrClassTokens({ key: value })` внутри статического
  значения (` key` для каждого истинного), либо `ssrClass({ … })` после атрибутов, если статического
  `class` нет. Hydrate совпадает с client: первый запуск привязки переключает токены в то же состояние.
...
- Текстовый прогон: соседние текст и выражения, вид которых известен (`static_kind`, §7.12; условие,
  которое мемоизируется, не входит), образуют один текстовый узел шаблона, если в прогоне есть выражение и
  его текст никогда не пуст — есть непустой статический текст или все выражения `Numeric`. В шаблоне узел
  держит статический текст (или один пробел-заместитель, если статического нет); привязка пишет
  `node.data = "статика" + (выражение) + …` (один раз, если ничто не реактивно). Server выводит статический
  текст с `ssrChild(выражение)` на местах выражений, без скобок `<!--[-->` и без заместителя; hydrate
  находит узел как обычный статический ребёнок. Иначе выражения остаются вставками.
...
### 7.12 Статический вид значения

`types::static_kind` доказывает по синтаксису, что выражение всегда даёт число/bigint (`Numeric`, текст
никогда не пуст) или строку (`String`); при сомнении — `None`:

- `Numeric`: числовой литерал; унарные `-`, `+`, `~`; бинарные `- * / % ** | & ^ << >> >>>`; `a + b`,
  где обе части `Numeric`; `Number(x)` с глобальным `Number`;
- `String`: строковый и шаблонный литерал; `a + b`, где хотя бы одна часть `String`; `String(x)` с
  глобальным `String`; вызовы `.toString() .toFixed() .join() .toUpperCase() .toLowerCase() .trim()
  .padStart()` (не опциональные);
- условное `c ? a : b` — вид веток, если он у них одинаков; чтение свёрнутого геттера (O3, §15.5) — вид его
  литерального инициализатора.
...
### O6. Автоматический selector в `<For>` (`optimize`)

В строке `<For>` сравнение `S() === K` или `K === S()` (также `!==`) переписывается в чтение selector-а,
если одновременно:

- элемент — `For` рантайма (по символу, §8.0; `For` импортируется из `reze-js` или `@rezejs/dom/list`), а
  его дети (или атрибут `children`) — стрелка или функциональное выражение (callback строки);
- `S` — голый вызов `S()` без аргументов геттера `signal`/`computed`, объявленного вне callback-а строки;
  не свёрнутый (O3, §15.5) и не инлайненный (O4, §16.5);
- `K` построен только из вызовов параметров callback-а без аргументов (`row()`, `index()`), статических
  чтений членов (`row().a.b`) и литералов, и читает хотя бы один параметр;
- ближайшая функция, содержащая сравнение, — сам callback строки (не вложенный обработчик).

Переписывание (client и hydrate): сравнение → `_sel$(K)`, `!==` → `!_sel$(K)`; вызов компонента →
`((_sel$, …) => createComponent(For, …))(selector(S), …)`, один selector на каждый различный `S`. Selector
создаётся один раз, в владельце, где вычисляется `<For>`, и живёт столько же. Server оставляет сравнение
как есть. Каждое переписывание даёт `info` `AUTO_SELECTOR`.

Рантайм: `selector(source, equals?)` из `@rezejs/signals` возвращает `isSelected(key)`; вызвавший
подписывается только на результат своего ключа, поэтому смена `S` перезапускает две строки, а не все.
...
### 15.16 Свёртка константных props (`optimize`)

Проп `k` компонента программы `C` сворачивается, если одновременно: `C` не убегает (§15.4); каждое
использование `C` в программе — тег JSX (`<C …/>`, не значение, не член пространства имён); ни у одного
тега нет spread; каждый тег передаёт `k` ровно один раз одним и тем же литералом — строкой или целым числом
(`k="x"`, `k={"x"}`, `k={1}`). Дети считаются атрибутом `children` без литерала.

Тогда в `C` каждое чтение `props.k` значением (первый параметр — идентификатор) и каждое чтение
деструктурированного `k`, которое переписывает §15.7, заменяется на `(литерал)`; для шаблона чтение —
литерал §7.10 (текст, атрибуты, мёртвые ветки O5, `static_kind`). Модули, экспортирующие `C`, становятся
замкнутыми (§15.3). Каждый свёрнутый проп даёт `info` `PROP_FOLDED` со ссылками на места вызова.

Сводка модуля хранит `tag_props`: для каждого тега JSX, названного привязкой модуля, ключи атрибутов и
литерал (`text` — как он выводится, `source` — JS), и есть ли spread.
